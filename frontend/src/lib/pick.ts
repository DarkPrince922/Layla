// Выбор элемента в превью: подсветка под курсором, клик — описание элемента родителю.
// Скрипт живёт внутри превью (изолированный iframe), с Лейлой общается только postMessage.
...
/** HTML превью со скриптом выбора (в файлы проекта он не попадает). */
...
/** Задача агенту: что за элемент и что с ним сделать. */
export function pickMessage(element: PickedElement, request: string, file?: string): string {
  return [
    `Измени выбранный на странице элемент${file ? ` (файл ${file})` : ""}.`,
    `Элемент: ${element.selector}`,
    element.text ? `Его текст: «${element.text.slice(0, 200)}»` : "",
    "Его HTML сейчас (фрагмент):",
    "```html",
    element.html,
    "```",
    `Что сделать: ${request.trim()}`,
    "Найди этот элемент в исходниках (read_file) и правь точечно через edit_file; остальное не трогай.",
  ].filter(Boolean).join("\n");
}
